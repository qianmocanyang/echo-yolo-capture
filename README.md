...
- **修复图片库首次打开缩略图挤成横带**（第二次打开又正常）：三处叠加——网格缺滚动区
  导致行高被强压到约 52px、列数按页面宽度而非容器宽度计算、旧格子只解除布局管辖却没
  真正移除导致新旧网格叠在一起
- **图片库翻页提速**：切页同步阻塞 239ms → 22ms。缩略图解码移到后台线程池（QImage 可
  跨线程，QPixmap 不行），格子分帧创建，每帧 12 个
- **高 DPI 下缩略图不再发虚**：系统缩放 125% 时按物理像素解码并写回缩放比（140 → 175），
  此前按逻辑尺寸解码后被系统放大 1.25 倍，细线和小字发糊
- **修复右栏整片控件被静默裁掉**：保存路径是 Windows 长路径，中间没有空格，
  `setWordWrap` 断不开，把侧栏内容顶到 590px 而视口只有 334px，水平滚动条又是关掉的，
  于是「精确坐标」按钮、Y 输入框、「锁定宽高比」直接看不见。路径标签改为中间省略显示
  （`ElidedLabel`），侧栏宽度 344 → 400（原值漏算了纵向滚动条占的 10px）
- **新增第六道自检**：`tools/check_layout.py` 在六档窗口尺寸下断言「右栏内容不超出视口 /
  图片库格子不被压扁 / 缩略图 DPR 与屏幕一致」，正是上面三轮界面 bug 的判据
